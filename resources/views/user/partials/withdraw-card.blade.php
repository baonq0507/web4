<div class="bg-[#232425] p-4 rounded-lg">
    ...
    <div class="flex justify-between items-center mb-2">
        <span class="text-white font-medium">{{ __('index.date') }}: </span>
        <span class="text-sm text-white/70">{{ $item->created_at->format('d/m/Y H:i:s') }}</span>
    </div>

    <div class="flex justify-between items-center mb-2">
        <span class="text-white">{{ __('index.phone') }}:</span>
        <span class="text-white">{{ $item->user->phone }}</span>
    </div>
    <div class="flex justify-between items-center mb-2">
        <span class="text-white">{{ __('index.method') }}:</span>
        <span class="text-white">{{ $item->payment_type == 'bank' ? __('index.bank') : USDT }}</span>
    </div>
    <div class="flex justify-between items-center mb-2">
        <span class="text-white">{{ __('index.amount') }}:</span>
        <span class="text-white">{{ number_format($item->amount, 2, ',', '.') }} USDT</span>
    </div>
    <div class="flex justify-between items-center">
        <span class="text-white">{{ __('index.status') }}:</span>
