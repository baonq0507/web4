    ...
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill font-size-h2 font-w400 mt-2 mb-0 mb-sm-2">{{ __('index.project_list') }}</h1>
                <a href="{{ route('cpanel.projects.create') }}" class="btn btn-alt-success my-2">
                    <i class="fa fa-fw fa-plus mr-1"></i> {{ __('index.add_project') }}
                </a>
            </div>
        </div>
    </div>
                ...
                <table class="table table-bordered table-striped table-vcenter" id="dataTable">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" style="width: 10%;">{{ __('ID') }}</th>
                            ...
                            <th class="text-center" style="width: 20%;">{{ __('index.profit') }}</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center"><img src="{{ asset($project->image) }}" alt="{{ $project->name }}" style="width: 100px; height: 100px;"></td>
                            <td class="text-center">
                                <span class="badge badge-{{ $project->status == 'active' ? 'success' : ($project->status == 'inactive' ? 'secondary' : 'danger') }}">{{ $project->status == 'active' ? __('index.active') : ($project->status == 'inactive' ? __('index.inactive') : __('index.stop')) }}</span>
                            </td>
                            ...
                            <td class="text-center">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <a class="dropdown-item" href="{{ route('cpanel.projects.edit', $project->id) }}" title="{{ __('index.edit') }}">
                                            <i class="fa fa-edit text-primary"></i> {{ __('index.edit') }}
                                        </a>
                                        <button class="dropdown-item btn-delete" data-id="{{ $project->id }}" title="{{ __('index.delete') }}">
                                            <i class="fa fa-trash text-danger"></i> {{ __('index.delete') }}
                                        </button>
                                        @if ($project->status == 'active')
                                        <button class="dropdown-item btn-stop" data-id="{{ $project->id }}" title="{{ __('index.stop') }}">
                                            <i class="fa fa-stop text-danger"></i> {{ __('index.stop') }}
                                        </button>
                                        @else
                                        <button class="dropdown-item btn-start" data-id="{{ $project->id }}" title="{{ __('index.start') }}">
                                            <i class="fa fa-play text-success"></i> {{ __('index.start') }}
                                        </button>
                                        @endif
                                    </div>
                                </div>
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('cpanel.symbols.store') }}" method="post" id="form-add-symbol" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="profit_win">{{ __('index.profit_win') }}</label>
                            <input type="number" class="form-control" id="profit_win" name="profit_win" required>
                        </div>
                        <div class="form-group">
                            <label for="profit_lose">{{ __('index.profit_lose') }}</label>
                            <input type="number" class="form-control" id="profit_lose" name="profit_lose" required>
                        </div>
                        <div class="form-group">
